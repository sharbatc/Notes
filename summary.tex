...
\section{Read the Stats carefully (TO BE CHANGED)}
\subsection{???}
Slide 2: \\
median = 50\% quartile. Very important measure. Sometimes more than the mean.
\\\\
Robust Statistics: \\
Statistic is robust if it's {\bf not sensitive to outliers}. Mean, std not robust. Median, quartiles are robust. Mean close to median implies few outliers. 
\\\\
Anscombe's quartet:\\
Slide 4: Outliers in plot 3 and 4. (Not enough to have only descriptive stat. Need visualization)
Illustrates the importance of looking at a set of data {\bf graphically}. See Wikipedia (LINK)
\\\\
Simpson's paradox:\\
UC Berkeley gender bias: Left is aggregate table. Women tended to apply to competitive departments => aggregate: they have a lower acceptance rate.\\
If trend appears in different groups, it can {\bf disappear or reverse when the groups are combined}. 
\subsection{Refresh of basic stats}
Proba: mathematical theory that describes uncertainty. \\
Stats: Set of techniques for extracting useful info from data. \\
They are close, but not exactly the same. Remember: Bayes theorem, random variables, law of large numbers, etc.
\\\\
Measurement on Samples: \\
Many datasets are samples from an infinite population. We have acces only to a {\bf sample} of a whole population. Therefore, measurements are hard because:
\begin{itemize}
 \item Samples measurements have {\bf variance}: Variation between samples
 \item Samples measurements have {\bf bias}: Systematic variation from the population value. (Thermometer say temp is 10 degrees more than pop)
\end{itemize}
Normal Distribution, Mean and Variance: \\
Mean is the average of all values. Variance measure the width of a distribution. (EQUATION) Standard deviation = sqrt of variance. (IMAGE?) 
\\\\
Central Limit Theorem: \\
Distribution of the sum (or mean) of a set of n identically-distributed random variables Xi {\bf approaches a normal distribution as n  $->\infty$.} \\
Common parametric statistical test, like t-test and ANOVA, assume normally-distributed data, but {\bf depend on sample mean and variance measures of the data.} \\
They typically work reasonably well for data that are not normally distributed as long as the samples are not too small!
\\
Often the data are not normally distributed. (But the stat tools assume that.) Therefore, check the data!
\\\\
Distributions: \\
\begin{description}
 \item[Poisson] Distrib of counts that occur at a certain rate.
 \begin{itemize}
  \item Observed frequency of a given term in corpus.
  \item Number of visits to a web site in a fixed time interval.
  \item Number of website clicks in an hour.
 \end{itemize}
 \item[Exponential] Interval between two such events.
 \item[Zipf/Pareto/Yule distrib] govern the frequencies of different terms in a document, or web site visits.
 \item[Binomial/Multinomial] govern the number of counts of event (e.g. die tosses = 6) out of $n$ trials.
\end{description}
